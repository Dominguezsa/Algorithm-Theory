...
def independent_set_bt(grafo, vertices, v_actual, puestos, n):
    ...
    if independent_set_bt(grafo, vertices, v_actual + 1, puestos, n):
        return True
    puestos.remove(vertices[v_actual])
    return independent_set_bt(grafo, vertices, v_actual + 1, puestos, n)


def ubicacion(grafo, n):
    puestos = set()
    vertices = list(grafo.nodes)
    independent_set_bt(grafo, vertices, 0, puestos, n)
    return puestos
...
def ubicacion_todos(grafo, n):
    vertices = list(grafo.nodes)
    return _ubicacion_bt_todos(grafo, vertices, 0, set(), n)


if __name__ == "__main__":
    inicio = time.time()
    print(ubicacion_todos(n_reinas(DIMENSION), DIMENSION))
    fin = time.time()
